...
def acc_roll():
    ...
    roll = math.degrees(math.atan(a_x/a_z))
    return roll
...
def acc_pitch():
    ...
    pitch = math.degrees(math.atan(a_y/a_z))
    return pitch
...
def x_speed(previous_x,current_x,prev_time,current_time=time.time()):
    elapsed = current_time - prev_time
    dt = [0,elapsed]
    data = [previous_x,current_x]
    x_speed = integrate.trapz(dt,data)
    return x_speed

def y_speed(previous_y,current_y,prev_time,current_time=time.time()):
    elapsed = current_time - prev_time
    dt = [0,elapsed]
    data = [previous_y,current_y]
    y_speed = integrate.trapz(dt,data)
    return y_speed

def z_speed(previous_z,current_z,prev_time,current_time=time.time()):
    elapsed = current_time - prev_time
    dt = [0, elapsed]
    data = [previous_z, current_z]
    z_speed = integrate.trapz(dt, data)
    return z_speed




def acc_x_displacement(previous_x_speed,current_x_speed,prev_time,current_time=time.time()):
    elapsed = current_time-prev_time
    dt = [0,elapsed]
    data = [previous_x_speed,current_x_speed]
    x_displacement = integrate.trapz(dt,data)
    return x_displacement

def acc_y_displacement(previous_y_speed,current_y_speed,prev_time,current_time=time.time()):
    elapsed = current_time-prev_time
    dt = [0,elapsed]
    data = [previous_y_speed,current_y_speed]
    y_displacement = integrate.trapz(dt,data)
    return y_displacement

def acc_z_displacement(previous_z_speed,current_z_speed,prev_time,current_time=time.time()):
    elapsed = current_time-prev_time
    dt = [0,elapsed]
    data = [previous_z_speed,current_z_speed]
    x_displacement = integrate.trapz(dt,data)
    return x_displacement
...
def gyro_roll(previous_y,current_y,elapsed): #getting gyro data
    elapsed = elapsed
    dt = [0,elapsed]
    data = [previous_y,current_y]
    area = integrate.trapz(dt,data)
...
def gyro_yaw(previous_z,current_z,elapsed): #getting gyro data
    elapsed = elapsed
    dt = [0,elapsed]
    data = [previous_z,current_z]
    area = integrate.trapz(dt,data)
